...
class Fighter extends SpriteActor {
    ...
    update(gameInfo, input) {
        ...
        if(input.getKey('ArrowUp')) { this.y -= this._speed; }
        if(input.getKey('ArrowDown')) { this.y += this._speed; }
        if(input.getKey('ArrowRight')) { this.x += this._speed; }
        if(input.getKey('ArrowLeft')) { this.x -= this._speed; }
